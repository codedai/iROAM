...
@skip_if_no_lightgbm
def test_vendor_bundle_loads_and_scores_bundled_example(monkeypatch):
    """Vendor bundle is still loadable via BUNCHING_MODEL_DIR override.

    The bundled ``example_input.npy`` is specifically the vendor 60×9 sample;
    we force the singleton loader to that bundle for this assertion. The new
    default bundle has different geometry — see the next test for that.
    """
    monkeypatch.setenv("BUNCHING_MODEL_DIR", str(BUNDLE_ROOT / "model"))
...
@skip_if_no_lightgbm
def test_default_bundle_loads_and_advertises_metadata():
    """Default bundle (locally retrained, rich features) loads + predicts shape."""
    from apps.api.services.bunching_predictor import get_predictor, reset_cache

    reset_cache()
    predictor = get_predictor()
    # Don't pin geometry — the bundle on disk drives it. Just check we got a
    # sane predictor we can call.
    assert predictor.pred_len > 0
    assert predictor.seq_len > 0
    assert predictor.n_channels >= 9
    assert predictor.metadata.get("trained_at")
    # Synthesize a zero-window of the right shape and confirm it scores cleanly.
    window = np.zeros((predictor.seq_len, predictor.n_channels), dtype=np.float32)
    probs = predictor.predict_proba(window, is_scaled=True)
    assert probs.shape == (1, predictor.pred_len)
    assert np.isfinite(probs).all()
